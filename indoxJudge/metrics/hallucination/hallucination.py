...
class Hallucination:
    def __init__(self, llm_response: str, retrieval_context: list, threshold: float = 0.5, include_reason: bool = True,
...
#
# class Hallucination:
#     def __init__(self, llm_response: str, retrieval_context: List[str], threshold: float = 0.5, include_reason: bool = True,
#                  strict_mode: bool = False):
#         self.threshold = 0 if strict_mode else threshold
#         self.include_reason = include_reason
#         self.strict_mode = strict_mode
...
#     def _generate_verdicts(self) -> List[HallucinationVerdict]:
#         verdicts = []
#         for context in self.retrieval_contexts:
#             prompt = HallucinationTemplate.generate_verdicts(actual_output=self.llm_response, contexts=context)
#             response = self._call_language_model(prompt)
#             data = json.loads(response)
#             verdicts.extend([HallucinationVerdict(**item) for item in data["verdicts"]])
#         return verdicts
#
#     def _generate_reason(self) -> str:
#         if not self.include_reason:
