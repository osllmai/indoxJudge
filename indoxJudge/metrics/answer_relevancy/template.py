...
"""
    ...
    The 'verdict' key should STRICTLY be either a 'yes', 'idk', or 'no'. Answer 'yes' if the statement is relevant to addressing the original input, 'no' if the statement is irrelevant, and 'idk' if it is ambiguous or partially relevant (e.g., not directly relevant but could be used as a supporting point to address the input).
    ...
    **          
    ...
    def generate_reason(irrelevant_statements, query, score):
        return f"""Given the answer relevancy score, the list of reasons of irrelevant statements made in the actual output, 
        and the input, provide a CONCISE reason for the score. Explain why it is not higher, but also why it is at its current score.
