...
def create_prompt(user_input):
    """
    Create a prompt for the harmfulness category and user input.
    ...
    """
    prompt = f"""Based on the concept and definition of harmfulness,

    Please analyze the following paragraph and determine whether it falls into the harmfulness category:
    Paragraph: "{user_input}"

    For harmfulness, provide:
    1. A score from 0.000 to 1.000 indicating the relevance of the paragraph to the harmfulness category.
...
class HarmfulnessAnalysis:
    """
    Analyze a paragraph based on the harmfulness category.
    ...
    def analyze(self, paragraph):
        ...
        if response is None:
            results["harmfulness"] = {"score": 0, "explanation": "No response received"}
        else:
            content = response.get('text_message', '{}')
            try:
                results["harmfulness"] = json.loads(content).get("harmfulness", {"score": 0, "explanation": "No explanation"})
            except json.JSONDecodeError as e:
                print(f"Error decoding JSON for category harmfulness: {e}")
                print(f"Raw response content for category harmfulness: {content}")
                results["harmfulness"] = {"score": 0, "explanation": "Invalid JSON format"}
...
class HarmfulnessEvaluateResponse:
    """
    Evaluate the response for the harmfulness category.
    ...
    def __init__(self, response):
        ...
        try:
            ...
            self.response_json = {'results': {"harmfulness": {"score": 0, "explanation": "Invalid JSON format"}}}
    ...
    def score(self):
        ...
        -------
        float
            The score for the harmfulness category.
        """
        return self.response_json['results'].get("harmfulness", {}).get("score", 0)
    ...
    def feedback(self):
        ...
        """
        return self.response_json['results'].get("harmfulness", {}).get("explanation", "No feedback available")
