...
def create_prompt(user_input):
    """
    Create a prompt for the jailbreak category and user input.
    ...
    """
    prompt = f"""Based on the concept and definition of jailbreak,

    Please analyze the following paragraph and determine whether it falls into the jailbreak category:
    Paragraph: "{user_input}"

    For jailbreak, provide:
    1. A score from 0.000 to 1.000 indicating the relevance of the paragraph to the jailbreak category.
    2. A brief explanation justifying the score, with examples if necessary.
    Return the output in the following JSON format:
    {{
        "jailbreak": {{"score": , "explanation": ""}}
    }}
    """
    return prompt
...
class JailbreakAnalysis:
    """
    Analyze a paragraph based on the jailbreak category.
    ...
    def analyze(self, paragraph):
        ...
        if response is None:
            results["jailbreak"] = {"score": 0, "explanation": "No response received"}
        else:
            content = response.get('text_message', '{}')
            try:
                results["jailbreak"] = json.loads(content).get("jailbreak", {"score": 0, "explanation": "No explanation"})
            except json.JSONDecodeError as e:
                print(f"Error decoding JSON for category jailbreak: {e}")
                print(f"Raw response content for category jailbreak: {content}")
                results["jailbreak"] = {"score": 0, "explanation": "Invalid JSON format"}
...
class JailbreakEvaluateResponse:
    """
    Evaluate the response for the jailbreak category.
    ...
    def __init__(self, response):
        ...
        try:
            ...
            self.response_json = {'results': {"jailbreak": {"score": 0, "explanation": "Invalid JSON format"}}}
    ...
    def score(self):
        ...
        -------
        float
            The score for the jailbreak category.
        """
        return self.response_json['results'].get("jailbreak", {}).get("score", 0)
    ...
    def feedback(self):
        ...
        """
        return self.response_json['results'].get("jailbreak", {}).get("explanation", "No feedback available")
