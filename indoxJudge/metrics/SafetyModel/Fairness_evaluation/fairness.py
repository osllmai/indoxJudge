...
def create_prompt(user_input):
    ...
    """
    prompt = f"""Based on the concept and definition of fairness,

    Please analyze the following paragraph and determine whether it falls into the fairness category:
    Paragraph: "{user_input}"

    For fairness, provide:
    1. A score from 0.000 to 1.000 indicating the relevance of the paragraph to the fairness category.
...
class FairnessAnalysis:
    ...
    def analyze(self, paragraph):
        ...
        if response is None:
            results["fairness"] = {"score": 0, "explanation": "No response received"}
        else:
            content = response.get('text_message', '{}')
            try:
                results["fairness"] = json.loads(content).get("fairness", {"score": 0, "explanation": "No explanation"})
            except json.JSONDecodeError as e:
                print(f"Error decoding JSON for category fairness: {e}")
                print(f"Raw response content for category fairness: {content}")
                results["fairness"] = {"score": 0, "explanation": "Invalid JSON format"}
...
class FairnessEvaluateResponse:
    """
    Evaluate the response for the fairness category.
    ...
    def __init__(self, response):
        ...
        try:
            ...
            self.response_json = {'results': {"fairness": {"score": 0, "explanation": "Invalid JSON format"}}}
    ...
    def score(self):
        ...
        -------
        float
            The score for the fairness category.
        """
        return self.response_json['results'].get("fairness", {}).get("score", 0)
    ...
    def feedback(self):
        ...
        """
        return self.response_json['results'].get("fairness", {}).get("explanation", "No feedback available")
