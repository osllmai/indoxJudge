...
class Gruen:
    ...
    def preprocess_candidates(self) -> List[List[str]]:
        ...
        for candidate in self.candidates:
            ...
            sentences = sent_tokenize(candidate) if self.use_nltk else candidate.split('. ')
            processed_sentences = [
                sentence for sentence in sentences if len(sentence.translate(str.maketrans('', '', string.punctuation)).split()) > 1
            ]
    ...
    def get_lm_score(self, sentences: List[List[str]]) -> List[float]:
        ...
        for sentence in tqdm(sentences):
            if not sentence:
                ...
                continue
            score = sum(self._score_sentence(s, tokenizer, model) for s in sentence) / len(sentence)
    ...
    def _load_pretrained_cola_model(self, model_name: str):
        config = BertConfig.from_pretrained(model_name, num_labels=2, finetuning_task='CoLA')
        tokenizer = BertTokenizer.from_pretrained(model_name, do_lower_case=True)
        model = BertForSequenceClassification.from_pretrained(model_name, config=config).to(device)
        model.eval()
        return tokenizer, model

    def _evaluate_cola(self, model, candidates: List[str], tokenizer, model_name: str) -> List[float]:
        dataset = self._load_and_cache_examples(candidates, tokenizer)
        dataloader = torch.utils.data.DataLoader(dataset, sampler=torch.utils.data.SequentialSampler(dataset), batch_size=max(1, torch.cuda.device_count()))
        preds = None
    ...
    def _load_and_cache_examples(self, candidates: List[str], tokenizer):
        examples = [InputExample(guid=str(i), text_a=c) for i, c in enumerate(candidates)]
        features = glue_convert_examples_to_features(examples, tokenizer, label_list=["0", "1"], max_length=128, output_mode="classification")
        all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long)
        all_attention_mask = torch.tensor([f.attention_mask for f in features], dtype=torch.long)
        all_labels = torch.tensor([0 for f in features], dtype=torch.long)
        all_token_type_ids = torch.tensor([[0.0] * 128 for _ in features], dtype=torch.long)
        return torch.utils.data.TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels)

    def _convert_sentence_to_paragraph_scores(self, sentence_scores: List[float], sent_length: List[int]) -> List[float]:
        paragraph_scores = []
        pointer = 0
    ...
    def get_grammaticality_score(self, processed_candidates: List[List[str]]) -> List[float]:
        """
        ...
        grammaticality_scores = [1.0 * math.exp(-0.5 * lm) + 1.0 * cola for lm, cola in zip(lm_scores, cola_scores)]
    ...
    def get_redundancy_score(self, all_summary: List[List[str]]) -> List[float]:
        ...
        for i, summary in enumerate(all_summary):
            if len(summary) == 1:
                continue
            flag = sum(self._if_two_sentence_redundant(summary[j].strip(), summary[k].strip())
                       for j in range(len(summary) - 1)
                       for k in range(j + 1, len(summary)))
            redundancy_scores[i] += -0.1 * flag
        return redundancy_scores

    def _if_two_sentence_redundant(self, a: str, b: str) -> int:
        """
        ...
        if max(len(a_split), len(b_split)) >= 5:
            ...
            if LCS_length > 0.8 * min(len(a), len(b)):
                flag_num += 1
            LCS_word_length = len(a[longest_common_substring[0]:(longest_common_substring[0] + LCS_length)].strip().split())
            if LCS_word_length > 0.8 * min(len(a_split), len(b_split)):
                flag_num += 1
            edit_distance = self._levenshtein_distance(a, b)
            if edit_distance < 0.6 * max(len(a), len(b)):
                flag_num += 1
            common_word_count = len([word for word in a_split if word in b_split])
            if common_word_count > 0.8 * min(len(a_split), len(b_split)):
                flag_num += 1
        return flag_num
