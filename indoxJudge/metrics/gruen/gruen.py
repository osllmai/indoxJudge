...
class Gruen:
    def __init__(self, candidates):
        """
        Initialize the Gruen class with candidate texts.

        Gruen is a text evaluation tool designed to assess the quality of text based on
        grammaticality, redundancy, and focus. It leverages language models to calculate
        scores for these metrics and aggregates them into a comprehensive evaluation.

        Parameters:
        candidates (Union[str, List[str]]): The candidate text(s) to evaluate. This can
                                            be a single string or a list of strings
                                            representing different candidate texts.
        """
    ...
    def get_focus_score(self, all_summary: List[List[str]]) -> List[float]:
        ...
        for summary in all_summary:
            ...
            for j in range(1, len(summary)):
                doc1, doc2 = nlp(summary[j - 1]), nlp(summary[j])
                try:
